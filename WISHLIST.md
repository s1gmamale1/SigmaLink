...
- **Replace false-green Electron/performance coverage (C-002/C-040/C-041/C-042).** Every launch must isolate profile, home, platform config roots, and workspace writes; smoke must unwrap/assert RPC results; perf must enforce repeated per-platform budgets and a pane create/close memory/child-count soak. Acceptance: CI fails on an unsafe profile, failed RPC/navigation, exceeded budget, or retained pane resources.

## P1 — Lifecycle and startup

- **Unify awaited child-tree shutdown (C-005/C-007/C-036).** Memory MCP, PTY descendants, ReviewRunner, and router shutdown need one TERM→bounded wait→tree KILL contract that finishes before database close. Acceptance: real trapped-child and descendant tests on POSIX plus Windows CI coverage leave zero owned descendants.
- **Remove the redundant resident Memory MCP child (C-023).** Keep a registry of launch commands or route clients through one owned server; do not spawn an idle copy per workspace. Acceptance: opening N workspaces adds no unused server processes and configured clients still connect.
