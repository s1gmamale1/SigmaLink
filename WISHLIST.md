...
- **[panes/C-class] structured Claude pane — opt-in DOM-chat pane type (the BridgeSpace counter)** — 2026-06-11 operator screenshot analysis of BridgeSpace v3.1.12 (Tauri + React + own terminal renderer + `osc133-parser.ts`, self-evidenced in their on-stream code review): their Claude panes are NOT terminals — Claude Code runs headless and the output stream is re-rendered as styled DOM chat (cards, chips, per-pane input strip). Resize there is pure CSS reflow: no PTY cols/rows contract, no SIGWINCH, no Ink repaint, no scrollback to duplicate — our entire 2026-06-11 bug class is impossible BY CONSTRUCTION. Counter-proposal: an opt-in pane type running `claude -p --output-format stream-json` (or Agent SDK) rendered with the existing Jorvis `ChatTranscript` streaming components — perfect reflow/scroll/copy for claude panes while PTY panes keep universal CLI fidelity (codex/gemini/vim/shells, which BridgeSpace's model can't host). Scope is REAL: tool-permission UI, slash-command surface, session resume continuity, interactive prompts. Their structural weakness stands regardless: all their agents share one working dir (our per-pane worktrees remain the edge — see `agentic-de-design-patterns`). Trigger: when claude-pane scroll-feel complaints persist after the #160 fullscreen-renderer dogfood, or when competitive pressure warrants the C-class investment. Effort: XL.
