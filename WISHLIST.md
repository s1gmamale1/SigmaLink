...
- **[pane-label] provider-agnostic task titles (injection + Haiku summarizer fallback)** — make every pane's LABEL a clean 2-4 word "what it's doing" title for ALL providers, not just Claude. Today only Claude self-titles (`SIGMA::LABEL` via `--append-system-prompt`); codex/gemini/cursor fall back to the operator's raw (often long) prompt — a poor title (operator-observed 2026-06-29: pane 1/claude titled correctly, panes 2-4 showed the literal prompt). Design (operator chose "both" + "titling…" placeholder):
  1. **Agent self-label (`SIGMA::LABEL`)** stays for Claude (free); extend the injection to codex/gemini/cursor via each CLI's own surface (`AGENTS.md` / `GEMINI.md` / `.cursor/rules` / a system-prompt flag) — **best-effort** (compliance varies; fragile per-provider research, and writing repo files is invasive).
  2. **Haiku summarizer fallback (the guarantee)** — on prompt-submit, if no clean `SIGMA::LABEL` lands within ~3s, a cheap Haiku call (reuse the in-app `assistant/` Anthropic client behind a new `rpc.paneTitle.summarize`) turns the prompt into a 2-4 word title. Provider-agnostic → clean title every time. **This layer alone solves correctness; the per-provider injection is only a cost optimization (can ship summarizer first).**
  3. **Orchestrator** (renderer, per pane): on prompt → `titling…` placeholder + 3s timer; a `SIGMA::LABEL` < 3s wins (cancel timer); else summarizer; a new prompt resets the cycle (re-title every prompt). Source priority: agent > summary > placeholder > launch-floor. The raw prompt is **no longer a sticky label**.
  4. KV toggle to disable the summarizer (cost/privacy — prompt text goes to Anthropic).
  Touchpoints: `src/shared/providers.ts` (per-provider injection) · `src/main/core/assistant/*` + a `rpc.paneTitle.summarize` IPC · `src/renderer/lib/pane-prompt-capture.ts` (feed the orchestrator, not the raw label) · new `pane-title-orchestrator.ts` · `PaneHeader.tsx` (`titling…` state) · `pane-labels.ts` (source-aware). Builds on PR #205 (name/label split). Effort: M.
