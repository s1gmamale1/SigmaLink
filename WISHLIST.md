...
- **[providers] Quote the provider install command before display and copy (C-059).** `Confirmed · medium · S`. Citations: `app/src/renderer/features/workspace-launcher/ProviderInstallModal.tsx:186,195`, `app/src/shared/providers.ts:243`. `cmd.join(' ')` renders and clipboard-copies an argv array unquoted, so cursor-agent's darwin/linux entry (`['bash','-c','curl https://cursor.com/install -fsS | bash']`) displays as `bash -c curl https://cursor.com/install -fsS | bash`. Pasting that runs `bash -c curl` with `$0` = the URL and `$1` = `-fsS`; curl prints usage, nothing installs, and `| bash` pipes the usage text into a shell. The operator sees a silent no-op. The in-app spawn path is unaffected — it passes the argv array to the PTY correctly. Action: format the display/copy string with the shared quoter from C-058. Verification trigger: the rendered string for every provider round-trips through a shell tokenizer back to the original argv array.
