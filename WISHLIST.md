...
## 🔬 Perf follow-up (2026-08-01) — opened by the arm64 baseline

- **[Perf] Phase 5's renderer decomposition is calibrated against a number ~5× the addressable working set (C-067).** `Confirmed · medium · M`. Citations: `app/docs/perf/2026-07-28-arm64-baseline.md` §3b, `ROADMAP.md` Phase 5 decomposition. The Phase 5 scrollback/trim constants were chosen against the **1684 MB** x64-under-Rosetta renderer footprint. The measured native-arm64 region table for the same heavy renderer shows 795.7 MB resident but only **335.9 MB dirty**, of which ~**261 MB** is V8/Blink heap (`Memory Tag 255`) — the only part app-side trimming can reach. `__TEXT` alone is 337.5 MB resident at **zero dirty**: shared, file-backed library code that inflates every per-process footprint reading and that no app change touches. **Why it matters:** a bound tuned to reclaim a fraction of 1684 MB is tuned against a figure that is roughly five times the reclaimable set, so the trim is likely either far more aggressive than needed (costing scrollback the operator wanted) or measured as "no visible saving" and then over-corrected. It is not a bug and nothing is broken — it is a calibration that was made before the measurement existed. Action: re-derive the decomposition from the measured region table before any further renderer tuning, and restate the Phase 5 target as "≈261 MB of V8/Blink heap", not "1684 MB". Verification trigger: a before/after `vmmap -summary` dirty-size delta on the heavy renderer at a **recorded, matched** pane count — footprint totals alone will not show it.
...
release; all were found by independent review, and each fails **open**.

C-061 · C-062 · C-064 · C-065 are **RESOLVED** in #254 (`9d92376f`). C-063 stays
**open by design**: the proposed fix does not compose (see its updated Action).

⚠️ Two release-aborting defects were found *in the C-064 fix itself*, both by
independent review, both invisible to a green suite AND to a real
`electron-builder --mac --dir` pack — the healthy tree is flat, so it never
enters the nested-resolution path at all. If you touch
`scripts/verify-packaged-deps.cjs`, exercise it with synthetic nested trees;
a green pack proves nothing about that code.

- ~~**[ram-brake] Extend the observed-process brake to the `+ Pane` path (C-061).**~~ → RESOLVED in #254 (`9d92376f`). The preflight moved out of `workspaces/launcher.ts` into a leaf module, `app/src/main/core/ram-brake/observed-preflight.ts`, because `factory-add-agent` cannot import the launcher (launcher → rpc-router → factory → factory-add-agent is a cycle). Both the workspace launch and `+ Pane` now run that ONE helper, with `force: input.forceRamBrake === true` threaded from `AddPaneButton`'s Force path. Fail-open and the `ramBrake.observedEnabled` kill switch live inside the helper, so the two call sites cannot drift. Tests: `swarms/factory-add-agent.observed-brake.test.ts` (hold · Force · null snapshot · rejected snapshot · `observedEnabled=0` takes no snapshots · foreign-workspace pane not attributed).
- ~~**[pty] `stop({tree:false, forget:true})` silently overrides its own opt-out (C-062).**~~ → RESOLVED in #254 (`9d92376f`). `forget()` gained a distinct `skipTree` hint, set by `stop()` when the caller passed `tree: false`. Kept separate from `treeAlreadyStopped` deliberately: that field states a FACT about the world ("the descendants are already down"), and reusing it to mean "don't touch the tree" would make its name a lie at one of its two call sites. Tests in `pty/registry-lifecycle.test.ts`: `{tree:false,forget:true}` performs no tree walk and kills the root exactly once; `{tree:true,forget:true}` still walks exactly once (no double-kill); a bare `forget()` (the RPC path) still tears the tree down.
- **[pty] `forget()` pays a synchronous process listing on the RPC path (C-063).** `Confirmed · low · M`. **Still open — the suggested fix does not compose.** Citations: `app/src/main/core/pty/registry.ts` (`forget()`'s `stopProcessTree`), `app/src/main/rpc-router.ts:1662` (`pty.forget` RPC), `app/src/main/core/process/process-tree.ts:31-33,116-127` (`execFileSync` — `ps -axo` on POSIX, `Get-CimInstance` on win32). Closing several live panes in quick succession on Windows blocks the Electron main process for a cold CIM query each time (routinely 0.5–2 s). The `stillAlive` guard spares the hot post-exit sweep, so the blast radius is bounded to live-session forgets. **Why the TTL-cached path is not the answer:** `inspectProcessTreeCached` only INSPECTS; the kill lives in `stopProcessTrees`, which re-lists synchronously and exposes no seam for pre-fetched rows. Feeding it a ≤2.5 s stale table would select SIGKILL targets from stale PIDs — `processSnapshotCached` is read-only by design and promoting it to a kill path is a different risk class. Writing an async killer inside `registry.ts` would duplicate the win32 `taskkill /T /F` and POSIX SIGTERM-leaves-first/SIGKILL-fallback logic — a second killer, which `registry.ts`'s own comment rejects. Batching does not help either: each pane close is its own macrotask seconds apart, so a same-tick batch never forms, and a debounced one trades away the no-stranded-descendants guarantee. Action: add `stopProcessTreesAsync` to `app/src/main/core/process/process-tree.ts` (async `execFile` for the listing; note win32 needs NO listing at all for the kill, since `taskkill /T` walks the tree itself and the snapshot there is purely informational), then route `forget()` through it. Verification trigger: closing 8 live panes on Windows shows no main-process stall.
- ~~**[packaging] The keep-list verifier checks presence-by-name, not resolvability (C-064).**~~ → RESOLVED in #254 (`9d92376f`). `findMissingModules` now judges presence the way Node's resolver does: a top-level hit (asar or either on-disk root) passes; a NESTED hit passes only when the owning package's `package.json` actually declares the dep, checked on disk and inside the asar. The asar surface gained `listAsarNodeModulePaths` + `splitPackagePath` because the old flat name Set could not express "top level vs nested under owner X". Deliberately still cannot false-FAIL — an unreadable owner manifest is accepted, since this hook aborts release builds. The failure message now names every location searched. Tests: `src/main/__tests__/verify-packaged-deps.test.ts` (undeclared nested namesake reported MISSING on disk AND in the asar; the real `@sigmalink/voice-whisper` → `node-gyp-build` shape still passes; two-owners-deep declared chain passes; unreadable manifest passes).
- ~~**[ram-brake] No `0`-means-disabled for the observed caps (C-065).**~~ → RESOLVED in #254 (`9d92376f`). Chose "reject loudly" over "document it": there is no settings UI for these caps, so a comment is invisible at the moment the operator sets the key, whereas a warn fires exactly then. A present-but-rejected value (`0`, negative, non-integer) now logs the key, the raw value, the fallback that applies, and points at `ramBrake.observedEnabled` as the real kill switch. An ABSENT key stays silent — that is the normal case. Behaviour is otherwise unchanged. Tests: `ram-brake/observed-preflight.test.ts`.
...
**✅ C-058 / C-059 / C-060 RESOLVED** in #253 (`5e997963`).
`app/src/shared/shell-quote.ts` is now the one quoter —
platform-aware *and* escaping — exposing `quoteShellArg` (always quotes, for
hand-interpolated values) and `joinShellCommand` (minimal quoting, for whole argv
arrays). All three sites route through it: `control-rpc.ts`'s `quoteArg` is a thin
alias, `provider-install.ts`'s local `shellJoin` is deleted, and
`ProviderInstallModal.tsx` formats both its display string and its clipboard copy
with it. Covered by `app/src/shared/shell-quote.test.ts` — a win32 × posix table
over {plain, space in path, embedded single quote, embedded double quote,
backslash-before-quote, trailing backslash, `$`/backtick/`;`/`|`, empty string}
for both functions, plus a round-trip through POSIX-`sh` and `CommandLineToArgvW`
tokenizers for every `AGENT_PROVIDERS` install command on all three platforms. The
posix connect-command byte-for-byte regression test is unchanged and still passes
(escaping is a no-op on values that contain no `'`). Entries kept below as history.
...
- **[P2 · release time] Choose one native rebuild owner (C-031).** `Confirmed · low · S`. Citations: `app/package.json:24-32`, `app/electron-builder.yml:29-30`, `.github/workflows/release-macos.yml:81-94`, `.github/workflows/release-windows.yml:74-91`. (`release-linux.yml` retired in v3.1.0.) Action: remove duplicate native rebuild ownership. Build trigger: clean release duration comparison plus native launch smoke tests.
...
- **[P1 · packaged CI] Launch built artifacts on every target OS (C-046).** `Confirmed · high · XL`. Citations: `.github/workflows/e2e-matrix.yml:83-103`, `.github/workflows/release-macos.yml:113-139`, `.github/workflows/release-windows.yml:94-118`. (`release-linux.yml` retired in v3.1.0.) Action: add packaged application launch gates after build/upload preparation. Build trigger: the macOS and Windows jobs exercise the actual packaged executable (Linux artefacts retired in v3.1.0).
