...
- **[ram-brake] Extend the observed-process brake to the `+ Pane` path (C-061).** `Confirmed · medium · S`. Citations: `app/src/main/core/swarms/factory-add-agent.ts` (calls `checkRamBrakeAdmission`, never `checkObservedProcessBudget`), `app/src/main/core/workspaces/launcher.ts:284` (the only call site). A workspace already holding two `@claude-flow/cli … mcp start` chains at 5 GiB is correctly blocked from a full launch, but `+ Pane` spawns another leaky agent with no hold — growing the exact multiplier #251 exists to contain. `AddPaneButton`'s hold-parsing is forward-compatible plumbing that cannot fire today. Action: call `checkObservedProcessBudget` in `factory-add-agent` with `force: input.forceRamBrake === true` (the plumbing is already threaded). Verification trigger: a leaky same-workspace pane holds `+ Pane` and the Force escape completes the add.
- **[pty] `stop({tree:false, forget:true})` silently overrides its own opt-out (C-062).** `Confirmed · low · S`. Citations: `app/src/main/core/pty/registry.ts:607` (`treeAlreadyStopped`), `:655` (`forget()`'s own `stopProcessTree`). With `tree:false` the snapshot stays null, so `treeAlreadyStopped` is false and `forget()` runs a full tree kill anyway. A caller that deliberately wants root-only teardown — leaving a long-running detached child alive — gets the whole descendant tree SIGKILLed. No production caller uses that combination today, so it is unreachable, but the opt-out is now a lie for the next caller. Action: thread the caller's `tree` intent into the hint, or add a distinct `skipTree`. Verification trigger: a test covering exactly `{tree:false, forget:true}` asserts descendants survive.
- **[pty] `forget()` pays a synchronous process listing on the RPC path (C-063).** `Confirmed · low · M`. Citations: `app/src/main/core/pty/registry.ts:655`, `app/src/main/rpc-router.ts:1662` (`pty.forget` RPC), `app/src/main/core/process/process-tree.ts:31-33,116-127` (`execFileSync` — `ps -axo` on POSIX, `Get-CimInstance` on win32). Closing several live panes in quick succession on Windows blocks the Electron main process for a cold CIM query each time (routinely 0.5–2 s), janking the whole Command Room on a teardown that used to be instant. The `stillAlive` guard spares the hot post-exit sweep, so the blast radius is bounded to live-session forgets. Accepted as the cost of not stranding descendants. Action: use the TTL-cached async path (`inspectProcessTreeCached`) for the RPC entry point, or debounce across a batched forget. Verification trigger: closing 8 live panes on Windows shows no main-process stall.
- **[packaging] The keep-list verifier checks presence-by-name, not resolvability (C-064).** `Confirmed · low · S`. Citations: `app/scripts/verify-packaged-deps.cjs:216` (`onDiskAnyDepth`), `:136` (depth-agnostic asar match). Node resolves upward only, so `node_modules/A` requiring `foo` never finds `node_modules/B/node_modules/foo` — but the guard accepts a hit at any depth. It can therefore false-*pass* (mask a genuinely missing module), never false-fail. Two ways in: a top-level copy vanishing while a nested one remains, or a shipped package containing a test fixture directory named `node_modules/<keep-list-entry>`. Mitigated by the now-pinned `nodeLinker: hoisted`. Action: require top-level presence first, accepting a nested hit only when the owning package's `package.json` actually declares the dep. Verification trigger: a fixture where only a nested same-named copy exists is reported MISSING.
- **[ram-brake] No `0`-means-disabled for the observed caps (C-065).** `Confirmed · low · S`. Citation: `app/src/main/core/workspaces/launcher.ts:122` (`readPositiveIntKv` requires `parsed > 0`). Setting `ramBrake.maxClaudeFlowStdioPerSession = 0` silently falls back to the default `1`. #251 added `ramBrake.observedEnabled` as the real kill switch, so this is now cosmetic — but a `0` cap reading as `1` is still surprising. Action: reject `0` loudly, or document the fallback at the KV surface. Verification trigger: a `0` cap either errors or is documented where an operator sets it.
