...
## v1.2.4 — Auto-update without signing

### Added
- macOS DMG manual update path: download → open DMG → drag to Applications
- Windows NSIS auto-update path: one-click quit-and-install via electron-updater
- Settings → Updates state machine with download progress
- Shared `http-download.ts` utility for atomic downloads with redirect following

### Changed
- `electron-builder.yml`: `verifyUpdateCodeSignature: false` on Windows (unsigned builds)
- `marketplace.ts`: refactored to use shared `http-download.ts`

### Fixed
- macOS releases v1.1.4–v1.1.11 missing `latest-mac.yml` manifest (now generated by CI)
