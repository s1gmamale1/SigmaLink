...
> **Pre-tag verification (done 2026-06-03 — headless real-environment smokes):** the destructive paths CI can't unit-test were exercised for real and **pass**: **DB-2** backup→restore→identical round-trip + both fail-safes (corrupt source throws & leaves the live DB intact; missing source throws) via real `better-sqlite3` under `ELECTRON_RUN_AS_NODE`; **FEAT-11** rewind in a real git worktree (rewind + recover-via-safety-checkpoint + divergent-reject, 10/10) — **this found & fixed a real bug, see Fixed below**; **responsive** layout sane at the 1024px floor + 1280px (no horizontal overflow, sidebar auto-collapse, the Settings tab-wrap reachable). `npm run test:perf` ran clean (CLS=0, 1 long task; flagged windows are benign room-mount frame gaps — video-confirmed) + a code audit found no must-fix (findings → PERF-15/16). The **live swarm-rail perf watch** (PERF-15) still wants an eyeball under real multi-agent streaming. **Tagged `v2.0.0` on 2026-06-07** after the operator confirmed the live in-place resume round-trip (PRs #127/#128) and Phase 9 completion (#125/#126) — see the top two sections below; the rest of this entry is the 2026-06-03 staging.

### Added — Phase 9: Orchestration & memory surfacing — Sigma panel, routing trace, 1-click graph, Automations, named sessions, Artifacts (2026-06-06/07, PRs #125 `7a51a69` + #126 `547da3c`)

The final feature arc, completing the roadmap. Built as disjoint isolated-worktree lanes + an Opus integration review per packet; full `tsc -b` · vitest · lint · build · CI `e2e-matrix` green.

- **9A — Sigma rail panel + routing trace + 1-click graph (BSP-O1/O2/O5, #125).** A persistent **"Sigma" right-rail tab** (`SigmaPanel`): a **Canvas** sub-tab with numbered swarm-agent to-dos (shared `deriveStatus` glyphs, matching the Swarm room) + a swarm-aggregated **live token delta** (`useSwarmLiveStats`, polled per-session, gated on the tab being the active rail tab); and a **Review** sub-tab with the **live routing/tool trace** (reuses `ToolCallInspector` on the `assistant:tool-trace` broadcast). Plus the **Ruflo memory graph made ≤1 click** from any room — a ⌘K "Open memory graph" command, a persistent breadcrumb button, and a `pendingMemoryGraphView` nonce (mirrors `pendingRufloView`). The Opus review backfilled the new room into `VALID_ROOMS`/`ROOM_DEFS`/`GLOBAL_ROOMS` (and the pre-existingly-missing `git`/`sigmabench`) so it restores-on-reload and is ⌘K-reachable.
- **9B — Automations room + named sessions + Artifacts (BSP-O3/O4, #126).** A **global "Automations" room** surfacing the existing Telegram remote bridge + nightly-digest scheduler as a live dashboard (status badges, inline enable toggles, Settings deep-links; reuses existing `rpc.telegram.*`/`rpc.kv.*`, **zero new channels**). **Per-conversation named sessions** — migration `0036` adds nullable `agent_sessions.name` + a `panes.rename` RPC (mirrors `setDisplayProvider`) + inline title-pill rename, surfaced through the shared `derivePaneIdentity` so the gear popover / context sidebar / splash reflect it. A per-session **Artifacts** view (changed files via `git.status` + checkpoint timeline via `git.listCheckpoints`; no new table).

### Fixed — in-place pane session resume: cross-pane hijack + ghost re-spawn loop (2026-06-07, PRs #127 `425594c` + #128 `24f462d`)

Found in live operator testing; **invisible to the e2e smoke** (in-place multi-pane resume isn't exercised). In an **in-place** workspace every pane shares the repo-root cwd, which broke session identity two ways:

- **#127 — session hijack.** A null/ghost `external_session_id` fell back to a "continue-latest" flag (`claude --continue` / `codex resume --last`); in the shared cwd that resolved to the cwd's newest session — the **operator's own running CLI session** or a sibling pane's — collapsing every pane onto one conversation. Now a null/ghost id starts a **fresh** session (never a continue-latest guess), the policy `gemini` already used (B2). Root-caused via live DB + on-disk session evidence (`/systematic-debugging`).
- **#128 — ghost re-spawn loop.** After #127, panes whose stored id had no on-disk JSONL spawned fresh on **every** reopen because boot-resume never captured the new id (`sessionId` + `isResume:true` suppresses capture). Now the fresh-fallback for deterministic-`--session-id` providers (claude) spawns with fresh semantics (`preassignedSessionId` + `isResume:false`), clears the stale ghost, and **persists the pre-assigned id** so the next reopen resumes by a real id. codex/others stay on the safe fresh-no-capture path (their cwd disk-scan races siblings/operator in a shared cwd). Both Opus-reviewed; +regression tests. **Operator-confirmed live** before tagging.

### Added — Phase 6: Premium Jorvis FE — streamed reveal + spring bubbles + inline tool chips (2026-06-05, PR #109 `8cd9ff6`)

The Jorvis assistant feels premium: a live reply reveals smoothly with a caret, each bubble springs in once, and in-flight tool calls render as inline chips. **Renderer-only** over the existing IPC contract — reuses the pure-CSS spring system (`--ease-snappy` `linear()` tokens + `animate-sl-*`), no new animation library. Built in an isolated worktree off `origin/main` + merged via PR (CI green), with an Opus adversarial review. Plan: `docs/superpowers/plans/2026-06-05-phase6-premium-jorvis-fe.md`.

- **rAF catch-up stream-reveal** (`use-jorvis-stream-reveal.ts`, new) — paces the accumulated reply text on screen with a trailing caret; reduce-motion returns the full text instantly (no caret, zero rAF); cancels its rAF on unmount/inactive. Only the in-flight assistant row reveals (turn-scoped via a `__streaming__` sentinel keyed by the eventual `messageId`).
- **First-mount-only spring bubble-enter** — React-19 ref-as-prop + `useLayoutEffect([])` + a `playedRef`, reusing `animate-sl-slide-up` (transform/opacity only). **No re-spring when the streamed bubble commits**: the sentinel row is keyed by the real `messageId` so React reuses the same DOM node across the stream→commit transition (the bubble stays put — "spring in once").
- **Inline per-turn tool-chip rail** (`InlineToolChips.tsx`, new) — subscribes to `assistant:tool-trace`, renders compact pills (tool name + ok/error dot + duration), bounded to the active turn by the mount lifecycle.
- **Recon-corrected scope:** the ROADMAP listed a "backend incremental-delta emit (today whole blocks)" prerequisite, but the **delta contract already existed** (`assistant:state`/`kind:'delta'`); `claude --output-format stream-json` only yields complete blocks per line (token-level deltas aren't a CLI capability), so the premium streaming feel is a renderer rAF job — **no backend rewrite needed**.
- **Reduce-motion** honored end-to-end (CSS global safety-net + `prefersReducedMotion()`); the existing 90s backend + 120s renderer **hung-turn watchdogs** are untouched (the reveal's rAF is independent and cannot keep a turn alive).
- Also fixed a **pre-existing `main` lint failure** inherited from Phase 4 (#107): `GridLayout.tsx` exported a non-component helper (`reshapeFracs`) → `react-refresh/only-export-components`; extracted it to a `grid-fracs.ts` leaf module, repairing `lint + build` CI for the whole repo.
...
- **PERF-3 — `useAppState` whole-context re-render storm.** The 5 hottest consumers (`RoomSwitch`, `MainBody`, `GlobalMemorySwitcher`, `TasksRoom`, `MemoryRoom`) plus the leaky whole-map `swarmMessages` subscribers (`SwarmRoom`/`SwarmRailTab`/`OperatorConsole`) now subscribe to individual slices via the existing `useAppStateSelector` (`useSyncExternalStore`); dispatch-only consumers use `useAppDispatch`. A dispatch no longer re-renders every consumer — only subscribers of the changed slice. Module-level empty-constant fallbacks keep referential stability; per-thread `swarmMessages` preserves the "already-loaded" distinction via an `undefined` sentinel. Render-count regression tests added.
- **PERF-8 — blocking disk scan.** `session-disk-scanner.ts` converted from synchronous `fs.*Sync` to `fs/promises` throughout (stat / readdir / open-read-close / readFile + the BFS `findFiles`); the public entrypoints already returned Promises, so the API shape is unchanged. The main-process IPC loop no longer stalls while enumerating session directories.
- **DEV-W3b — per-workspace in-place / no-worktree mode (ADR-007).** A new `shared/worktree-mode.ts` KV flag (`workspace.worktreeMode.${id}` = `worktree|in-place`, fail-safe default `worktree`) short-circuits **both** sibling worktree gates (`launcher.ts` + `factory-spawn.ts`) to the existing no-worktree path, so agents run in the repo root with **zero** worktree dirs. A Settings toggle writes the flag with an explicit concurrent-edit collision warning. Removes the Phase-0 disk class entirely for non-isolated workflows.
- **ruflo-observability — catch the next runaway early.** Structured `[worktree]` create-decision logs (count/cap/free/floor), an always-emit boot-sweep free-disk baseline, and a **critical `NotificationsManager` alert on every `WorktreeDiskGuardError`** at all **three** spawn sites (launcher / swarm-roster / +Pane). The Opus review caught that the alert was a silent no-op in production — the optional `notifications` sink was never threaded into the prod callers (`rpc-router` `executeLaunchPlan`/`buildSwarmController` → `controller`); fixed so a 49 GB-class incident would now ring the bell, not just `console.warn`.

**Deferred → `WISHLIST.md`:** the swarm-mode launcher UI doesn't expose the in-place toggle (backend honors it for swarm spawns either way); a queryable `worktree_events` table (C8); a `npm run test:perf` before/after jank measurement (operator smoke).

### Fixed — Phase 0 (RELEASE-BLOCKER): disk-fill + post-crash launch lockout + workspace loss (2026-06-05)

A local `electron:dev` session ballooned the worktree pool to **49 GB and filled the system disk**, forcing a force-quit — after which the app **could no longer launch any pane** (every spawn hit an `agent_sessions` UNIQUE violation → "duplicate spawn suppressed") and **silently lost which workspaces were open**. A 6-agent read-only `/systematic-debugging` fleet root-caused it: it was a leaked-git-worktree **disk** leak (not RAM) and **not** a Tauri-migration trigger (the same bug exists under any host language — ROADMAP ADR-006). Fixed across two file-disjoint lanes + a follow-up regression fix, and verified **three** ways — operator GUI force-quit→relaunch, the full gate, and a new automated crash-recovery smoke.

- **CRIT-1 — runaway disk consumption (Lane A, `8e203b2`).** `WorktreePool.create` now enforces a worktree-count cap **and** an `fs.statfs` free-disk floor (`WorktreeDiskGuardError`); a boot `sweepAllReposOnBoot` + `cleanupOrphanWorktrees` reaps orphaned checkouts. The leak source — the UNIQUE-suppression catch branches in `factory-spawn.ts` and `launcher.ts` created a git worktree (a full HEAD checkout, ballooned to 100s of MB once an agent ran `npm/bun install` inside it) and then never removed it — now `removeAndPrune`s the worktree the instant a spawn is suppressed (`42ee75f` + `f1b7ac8`).
- **CRIT-2 — `agent_sessions` UNIQUE lockout after a crash (Lane B).** The partial unique index `agent_sessions_ws_pane_uq` was status-agnostic, so force-quit zombies (left `running` with `pane_index` set) made the slot allocator hand back a slot the index still considered occupied → every fresh INSERT collided → permanent launch lockout (resume escaped only because it UPDATEs the existing row). Fixed with a **status-aware** index (migration `0032`, ADR-005 — unique only `WHERE pane_index IS NOT NULL AND status IN ('running','starting')`, `efbb472`); the boot janitor now also sweeps `starting` zombies while preserving `pane_index` (`2e1433b`); and `runBootJanitor` + `sweepAllReposOnBoot` are now **awaited before the window opens** so crash-recovery completes before any resume runs (`d384b0e`).
- **CRIT-3 — open workspaces silently lost on force-quit (Lane B, `b0c7725`).** `app.lastSession` was written only in `before-quit`, which a SIGKILL force-quit never runs → genuine data loss independent of the lockout. A throttled (~2 s trailing-edge, unref'd timer) opportunistic snapshot flush now persists the open-workspace set during normal operation, so a force-quit keeps them.
- **Pane-resume regression — black/dead panes after force-quit→relaunch (`93fbca6`).** Enabling the boot sweep (CRIT-1) surfaced a latent invariant violation: the reaper's keep-set (`running OR exited<7d`) was **narrower** than resume-launcher's eligibility (`running OR (exited AND exit_code=-1)`), so the sweep deleted worktrees that resume would re-spawn into → missing cwd → early death → black panes. Fixed by widening the keep-set to a **superset** of the resume-eligible set (also preserving `starting` + `exited/-1`). Root-caused + patched by Codex from a written brief (`docs/08-bugs/2026-06-05-pane-resume-crash-after-forcequit.md`), reviewed + integrated + re-gated, then **operator-GUI-confirmed live**.
- **Crash-recovery smoke (`d9f3ba4`).** A new Playwright smoke (`npm run test:smoke:crash` → `tests/e2e/crash-recovery.smoke.spec.ts`) drives spawn → SIGKILL → relaunch → asserts panes resume; it was **red before the fix, green after** — the first automated coverage for this entire failure class.

**Deferred (non-fatal → `WISHLIST.md`):** resume self-heal **Part B** (recreate-if-missing worktree before re-spawn — didn't bite in retest, since the live recovery succeeded) · the benign `ResizeObserver loop` error surfacing as a fatal toast (filter it in the global error sink).

### Fixed — Phase 1: SMK + DEV bugfix batch — sessions, skills, browser (2026-06-05, `cca05ad`)

The two most-used entry surfaces (the workspace-launch wizard and the Skills rail) and the embedded browser. Root-caused by a 3-agent recon fleet, fixed across **three file-disjoint worktree lanes** (TDD per bug, Opus two-stage spec→quality review each), integrated onto `main` and re-gated there (`tsc -b` · vitest **2764** · lint · build · full `tests/e2e/`). Each fix shipped with the regression test whose absence had hidden it. Plan: `docs/superpowers/plans/2026-06-05-phase1-bugfix-batch.md`.

- **SMK-2 — sessions-step selection reverted instantly (looked dead).** `Launcher.tsx` built `rows={buildPaneRows(...)}` inline → a new array identity every render → `SessionStep`'s `useEffect([rows,…])` self-refired through `setPaneResumePlan` and overwrote the user's pick with the smart default within a tick. Fixed by memoizing `paneRows` (`useMemo([counts, skipAgents, preset])`); added the first **un-stubbed** `Launcher↔SessionStep` integration test (the loop was invisible because `Launcher.test` stubs `SessionStep`).
- **DEV-4 — workspace rail reordered on click.** `SET_ACTIVE_WORKSPACE_ID` called `upsertOpenWorkspace` (prepends) → the activated workspace jumped to the top. Now it activates **in place**; `SET_ACTIVE_WORKSPACE` (open-new) still prepends.
- **SMK-3 — Skills tab was Superpowers-only (≈580+ skills invisible; latent ruflo bug).** New `core/skills/discovery.ts` scans **all** providers via the `installed_plugins.json` manifest (not a blind glob — the cache holds 80+ `temp_*` staging dirs) + flat dirs (claude user skills/commands, codex `$`, gemini); ruflo reached through its `<version>/` dir; source tagged by the **un-namespaced** manifest key (`<name>@<marketplace>` — a too-clean test fixture had hidden this). `InstalledSkillEntry` widened (`+provider +prefix`) consistently across all **3 mirrored sites** + `SOURCE_BADGE`.
- **SMK-3b — codex skill injection typed `/name`.** Per-provider prefix map; codex now gets `$name`, with `providerId` threaded from `PaneShell`.
- **DEV-1 — browser "Design" element-pick was a no-op.** The element dispatch wrote a trailing `\r` (auto-submit) → now `\n` (matching every other PTY writer); and element capture now **auto-seeds an editable prompt template** so the Dispatch button enables (the user still edits before dispatching).
- **DEV-2 — recently-closed browser tabs weren't persisted.** `closeTab` hard-deleted the row. Migration **`0033`** adds `browser_tabs.closed_at` (soft-delete, H-7 transactional); `closeTab` soft-deletes, hydrate excludes closed rows, and a new `listRecents` + `browser.listRecents` RPC surface closed-tab origins in Recents.
- **DEV-3 — address bar inert until a tab existed.** Typing a URL with no open tab now opens the first tab; the URL input is enabled while the nav buttons stay gated on an active tab.

**Scope notes.** **DEV-5** was **refuted** — it is SMK-2 observed across multiple panes (fixed by the SMK-2 memoization), not a separate code path. **SMK-1**'s `scoped` guard had already landed (B2 fix); its benign opencode same-cwd residual is deferred. Dev-infra **DEV-6/7/8** deferred. Reviewers' non-blocking follow-ups → `WISHLIST.md` (the closed-tabs table has no GC; the SMK-2 loop test fails via timeout rather than a fast assertion).

- **CI — the Phase-0 crash-recovery smoke's `node:sqlite` import made lazy (e2e-matrix stays on Node 20).** The smoke (`d9f3ba4`) imported built-in `node:sqlite` (needs Node ≥22.5) **statically at module top-level**; CI runs Node 20.x → the import crashed both `smoke` jobs at *collection* time (pre-existing red, unrelated to Phase 1) even though the test is already `SMOKE_CRASH=1`-gated and skipped there. Fixed by lazy-loading `node:sqlite` via `createRequire` inside the DB-read helpers, so the file imports cleanly on Node 20 (test skips → job green) and still runs on a Node ≥22.5 host under `SMOKE_CRASH=1`. (An attempted host-Node bump to 24.x was reverted — it broke Electron's CI install across the *entire* e2e suite.)

### Fixed — FEAT-11 rewind: the pre-rewind safety checkpoint is now restorable (redo)

A pre-tag live-git smoke found that `git.restoreCheckpoint`'s guard was **ancestor-only**, so the auto "pre-rewind" safety checkpoint (a *descendant* of the rewound HEAD) could not be restored — clicking **Restore** on the amber `auto` row in `CheckpointPanel`, which the confirm dialog advertises as *"this rewind can itself be undone,"* failed with "not an ancestor." (Work was never lost — the safety commit is in git's object store — but the one-click undo was broken.) The guard now accepts a checkpoint on this worktree's lineage in **either** direction — ancestor (rewind) or descendant (redo) — rejecting only a truly divergent/foreign sha; the controller's ownership guard already proves it belongs to this session. (Unit-tested with mocked `execCmd`, the ancestor-only assumption was invisible — the real-git smoke was the only signal.)
>
