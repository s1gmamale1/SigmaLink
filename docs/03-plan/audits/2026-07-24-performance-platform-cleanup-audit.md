...
| C-002 | perf/test safety | Nine of twelve Electron launch expressions use the operator's real profile and can migrate the live database, run janitors, restore workspaces, and respawn panes. | Confirmed source/data-flow trace | Electron test launch enumeration → `rpc-router.ts:444-509` → database/session restore | Fixed: all launch expressions use enforced isolated profile/home/workspace roots |
| C-003 | PTY/protocol memory | An unterminated PTY line can grow `ProtocolLineBuffer.acc` without a bound; every normal pane feeds one prompt-sink buffer and swarm panes feed a second protocol buffer. | Confirmed | `app/src/main/core/swarms/protocol.ts:112-130`; `app/src/main/core/pty/registry.ts:352-354`; `app/src/main/core/swarms/factory-spawn.ts:541-554` | Fixed: newest 64 KiB retained |
...
| C-037 | macOS environment | Interactive-login shell banner output is accepted as the discovered PATH and cached verbatim. | Confirmed injected parser path | `util/shell-path.ts:58-73,103`; `electron/main.ts:973` | Fixed: nonce-delimited PATH payload |
...
| C-040 | test isolation | Isolating only Electron `userData` still permits workspace `.mcp.json`/`CLAUDE.md` and operator home configuration writes. | Strong static evidence | `workspaces/factory.ts:324-371`; `workspaces/mcp-autowrite.ts:113-195`; `seed-workspace-memory.ts:115-150` | Fixed: helper owns home/platform roots and disposable workspaces |
...
| C-045 | E2E lifecycle | Several Electron tests close the application only on the success path, so failed assertions can leak processes and locks. | Strong static evidence | perf, smoke, sync, and dogfood specs | Fixed: helper teardown/finally across launch sites |
...
- **C-003:** `ProtocolLineBuffer` retains only the newest 64 KiB of an unterminated line. TDD receipt: 1 failing / 10 passing before the bound; 11 passing after.
...
- **C-034:** Windows native availability consults the loaded module's real `isAvailable()` result, so the synchronous bundled fallback stub no longer reports available. The real native module's Promise-based probe is cached; its pending state remains optimistic, so visible behavior still needs Windows device validation.
...
- **C-002/C-040:** one helper now owns disposable Electron user data, home, Windows app-data roots, XDG roots, and a workspace. All 12 launch expressions use it. Main boot under `NODE_ENV=test` refuses to register production services without both the explicit isolation marker and `--user-data-dir`.
- **C-045:** every launch is closed by `finally`, helper teardown, or the dogfood `afterEach`; roots are deleted only after the application close attempt.
