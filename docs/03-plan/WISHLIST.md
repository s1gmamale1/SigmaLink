...
> **P1+P2+P3+P4+P5 shipped (#70 / #72 / #73 / #75+#76 / #78+#79, untagged).** Next phase: **P6 — Competitive features & leapfrogs (FEAT-* + worktree leapfrogs)** per `ROADMAP.md`. Owed pre-tag: P5 perf-harness + resize live smokes.
...
### ✅ Performance & optimization — MOSTLY SHIPPED P5 (PRs #78+#79)
> ✅ PERF-1/2/3/4/5/6/10/11 + RSP-1 shipped. PERF-13 was shipped in P2 (MemoryGraph). **🔭 OPEN → WISHLIST/P5.2:** PERF-7 (Constellation settle), PERF-8 (async disk-scan off main thread), PERF-9 (consolidate pty:exit/link listeners into the bus), PERF-12 (bounded JSONL first-line read), PERF-14 (FTS5, P4.2), shared density scale. Owed pre-tag: `npm run test:perf` jank-delta + `pty:data` IPC-rate live measurement.
...
- ✅ **RSP-1** *(shipped P5 — Strategy A)* — Resizable Memory tri-column (react-resizable-panels) + per-workspace persisted sizes; shared `useBreakpoint`; per-workspace rail widths (Sidebar/RightRail kept their px splitters) + right-rail narrow auto-collapse. **OPEN (P5.2):** shared density scale (comfortable/compact/cozy); extending Resizable to the rail/main shell (Strategy B); min-window e2e.
...
> **🔭 P4.2 OPEN (deferred):** MEM-5 (aliases) · MEM-7 (unlinked mentions) · MEM-8 (templates) · MEM-9 (properties/outline editor, on the BUG-10 frontmatter groundwork) · PERF-14 (FTS5) · Ruflo causal-edge rendering (daemon read-API unverified) · global ⌘O from any room · graph tag-**dim** (vs prune) · daily-note agent-activity digest · DB-2 backup→restore round-trip operator smoke (no vitest coverage — better-sqlite3 ABI).
