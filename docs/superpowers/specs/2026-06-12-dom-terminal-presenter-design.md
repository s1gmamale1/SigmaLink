...
**Date:** 2026-06-12 · **Status:** P1a SHIPPED v2.4.0 (#162) · P1b SHIPPED v2.4.0 (#163) · **default flipped to `dom` v2.4.1 (#165, operator-directed — pulls P3's flip forward)** · dogfood polish v2.4.2 (#166: SGR wheel routing, cursor space-tracking, alt-screen no-wrap + full-height backgrounds) · **P1c SHIPPED #167 `c7c01bf` (untagged, rides next release): GridView cell-exact alt-screen + conditional #160 (claude inline for DOM panes via `xtermOnlyArgs` + shared `renderer-mode`) + context-menu renderer toggle — PHASE 1 COMPLETE.** REMAINING: P2 (mouse reporting, OSC-133 blocks, links/search) · P3 (default-flip deletions: WebGL/reveal/fullscreen-injection removal, xterm→legacy)
