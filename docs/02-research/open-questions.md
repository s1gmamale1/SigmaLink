...
Facts referenced on sigmamind.ai or in repos but not actually published in detail. The build phase will need to invent vs replicate.

## SigmaSpace UI / interaction model
...
## SigmaMemory
9. Parameter signatures of all 12 SigmaMemory MCP tools (only group names + tool names public; no schemas).
...
16. How `~/.sigmaspace/runtime.session` token rotates / expires.
...
28. How file ownership is recorded (lockfile? in-memory? in SigmaMCP?).
...
## Roadmap
45. The "Five products. One mission." line lists only 4 — what is the 5th product? (Likely SigmaMemory or SigmaSwarm but unclear.)
...
## Plugins / open source
48. Skill marketplace JSON format (`@sigmamind-plugins` registry — exact protocol).
...
## Bridge Assistant
56. Mentioned in pricing (Basic) and changelog (v3.0.8 voice flows) but no dedicated page — is it a separate product or SigmaVoice integration inside SigmaSpace?
