# SigmaMind / SigmaSpace / SigmaSwarm Glossary
...
- **L** = "Introducing SigmaSpace" launch video — `RG38jA-DFeM` (275 s)
- **V3** = "Vibe Coding With SigmaSpace 3" — `xKf0B6AEo9I` (1085 s)
- **G54** = "Testing GPT 5.4 With Agent Swarms In SigmaSpace" — `0UDqnhsy4GA` (1385 s)
...
|---|---|---|
| **SigmaMind** | Parent brand / company. Domain: `sigmamind.ai`. Discord at `sigmamind.ai/discord`. | L 00:09, D, V3 16:30 |
| **SigmaSpace** | The Agentic Development Environment (ADE) desktop app. Cross-platform; "shipped on Mac OS, stable on Mac, Windows has bugs being fixed." Built with Tauri ("SigmaSpace Tauri" referenced in V3 03:30+). | L 00:09, V3 03:30, G54 03:36 |
| **SigmaSwarm** | The multi-agent coordination product *inside* SigmaSpace. Lets agents communicate with each other and with the operator. Different from SigmaSpace which has *isolated* agents. | L 02:03, V3 05:48 |
...
| **SigmaVoice** | Voice-to-text tool used to dictate prompts directly into SigmaSpace. Pro-plan feature. | V3 16:35 |
| **SigmaJarvis** | A voice assistant that orchestrates agents (named separately from SigmaVoice). Basic-plan feature. Possibly an alias for the same voice stack — distinct names suggest two layers. | V3 16:30 |
| **SigmaMind MCP** | An MCP (Model Context Protocol) server included with the Pro plan. | V3 16:35 |
| **SigmaSpace V3** / **V3** | Major release marketed at the SigmaSpace 3 launch on Product Hunt. Coupon code `V3` for 50% off. | V3 entire video |
| **SigmaSpace Tauri** | Internal name for the desktop application binary; appears as a referenced project name when prompting Bridge. | V3 03:30 |
| **SigmaSpace app** | A React Native Expo mobile companion app, *built during* the V3 demo. Will dispatch actions (e.g. swarms) to the desktop SigmaSpace at V4 release. | V3 02:38 |
| **BridgeBench** | A benchmark SigmaMind publishes for ranking coding LLMs. | G54 02:35 |
...
| Term | Definition | Source |
|---|---|---|
| **Workspace** | A top-level tab in SigmaSpace. Can be a Bridge Space, Bridge Swarm, or Bridge Canvas instance. Tabs visible in launch thumbnail: `Workspace 1`, `Workspace 2`, `Workspace 3`. | L 01:24, V3 thumbnail |
...
| **Cursor** ("cursor agent" / "Cursor CLI") | Available as an agent in SigmaSpace. | L 01:50, V3 00:51 |
...
| **Built-in browser** | Multiple browser tabs inside SigmaSpace. Any link printed by an agent is clickable to open inside this browser. | V3 11:13 |
...
| **Built-in IDE** | Click any file path printed in a terminal pane to open it in SigmaSpace's editor. Create/add files inline. | V3 13:24 |
...
| **SigmaSwarm V3 harness** | New harness underpinning V3 swarms. | V3 05:50 |
...
| **`@` mention syntax** | In-prompt mention (e.g. `@SigmaSpace Tauri`) to bind a target project to a swarm prompt. | V3 06:48 |
...
| Term | Definition | Source |
|---|---|---|
| **Basic plan** | $20/mo. 5,000 credits. SigmaMind account, SigmaSpace, multi-agent swarms, SigmaJarvis voice assistant. | L 04:08, V3 16:30 |
| **Pro plan** | (price not stated in these clips). 12,500 credits. Adds SigmaMind MCP and SigmaVoice. | V3 16:34 |
| **`launch20`** | Coupon code at launch (March 2026): 20% off. | L 04:21 |
| **`V3`** | Coupon code at V3 launch (April 2026): 50% off forever. | V3 16:24 |
| **Bug bounty (Bitcoin)** | Pays Bitcoin for reported SigmaSpace bugs. | G54 16:45 |
...
| Term | Definition | Source |
|---|---|---|
| **ADE** | "Agentic Development Environment" — SigmaSpace's product category. | G54 03:38, AT 00:28 |
...
| **Bug bounty program** | Bitcoin-paying program for reporting SigmaSpace bugs. | G54 16:45 |
