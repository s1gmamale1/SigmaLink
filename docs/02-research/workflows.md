# SigmaSpace / SigmaSwarm — End-to-End User Workflows

Step-by-step workflows extracted from the four SigmaMind videos. Each step lists the implied UI element name (from the glossary) and cites the source video. Sources: **L** (launch), **V3** (SigmaSpace 3), **G54** (GPT 5.4 swarm test), **AT** (Agent Teams).

Notation: `[click]` = mouse, `[type]` = keyboard text entry, `[voice]` = SigmaVoice / SigmaJarvis dictation, `[drag]` = drag-and-drop, `[wait]` = passive observation.
...
3. **Set the project directory.** Two options:
   - `[type]` shell path: `cd ~/Desktop/sigmamind` (or similar).
...
## W2. Launching a SigmaSwarm — original (V2) flow
...
## W3. Launching a SigmaSwarm — V3 flow (additional fields)
...
3. **Goal / mission prompt.** [voice or type]. Supports `@` mentions to bind a target project (e.g. `@SigmaSpace Tauri`).
...
1. Open the right-hand **Browser** tab; navigate to a rendered page (e.g. `sigmamind.ai`).
...
3. The file opens in the **right-hand IDE tab** (a slim editor inside SigmaSpace).
...
1. Repeat W2/W3 several times — each `⌘T` opens a new SigmaSwarm tab.
2. Demonstrated tabs: 5 concurrent swarms (`Next.js SaaS`, `Browser FPS game`, `Flight simulator`, `Chat app`, `D3`).
3. Switch between them via the workspace tab strip at the top of SigmaSpace.
...
## W9. Voice-driven workflow (SigmaVoice + SigmaJarvis)

Source: V3 throughout, G54 implied.

1. Hold/press the SigmaVoice trigger and speak the prompt (works in any prompt input — Bridge agent, swarm mission, Design Tool, Operator Console DM).
2. Voice transcribes into the active prompt box.
3. Submit normally.

(Distinction between SigmaVoice and SigmaJarvis isn't fully clarified verbally; current best understanding: **SigmaVoice = STT engine** (Pro), **SigmaJarvis = a higher-level voice assistant that takes actions** (Basic+). Both are mentioned together with no separation of UI in the videos pulled.)

---

## W10. Adjusting Claude Code effort inside a SigmaSpace pane
...
(This is Claude Code's UX, not SigmaSpace's, but it's how the visible splash strings are produced.)
...
```
SigmaMind (account / billing)
  └── SigmaSpace (desktop ADE app)
        ...
        └── Voice surfaces: SigmaVoice (STT) + SigmaJarvis (assistant)
```
