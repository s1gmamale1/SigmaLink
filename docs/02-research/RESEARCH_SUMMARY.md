...
This research pass covered **39 distinct URLs** across sigmamind.ai, docs.sigmamind.ai, GitHub (bridge-mind org), and Anthropic's canonical Skills documentation (used for cross-check), producing a per-page brief in `docs/02-research/web-pages/` plus nine synthesis files. We catalogued **66 product features** across the SigmaMind ecosystem (SigmaSpace, SigmaSwarm, SigmaMCP, SigmaMemory, SigmaVoice, SigmaCode, BridgeWard, BridgeSecurity, BridgeBench, plus future-named BridgeUI / BridgeRemotion / BridgeMotion / Bridge Assistant), enumerated **22 SigmaMCP+SigmaMemory tool names** (10 SigmaMCP server tools with full parameter signatures, plus the 12 SigmaMemory tools by name only), captured **8 SigmaSpace + 8 SigmaVoice changelog entries**, and compiled the canonical **4-role / 8-rule SigmaSwarm protocol** alongside the parallel role taxonomies appearing in five other blog posts. Seven product images were downloaded locally to `web-images/`.

## Top 10 unknowns (most consequential blockers for the build phase)

1. **SigmaSwarm Rule 5–8 verbatim text** — landing page lists 8 rules but the deep-dive blog only details 4. Need the missing four to faithfully clone behavior.
...
3. **SigmaMemory tool parameter signatures** — only the 12 names exist publicly; no schemas, no return types, no error semantics.
4. **The 5th product** — roadmap says "Five products. One mission." but lists only four. (Likely SigmaMemory or SigmaSwarm.)
5. **SigmaSpace room shortcuts** — "Fast room switching" is named but no key binding given; only macOS basic shortcuts published.
...
10. **Bridge Assistant identity** — appears in Basic-tier pricing and changelog v3.0.8 but has no product page; unclear if it's a standalone surface or SigmaVoice-in-SigmaSpace.
...
- `mcp-tool-catalog.md` — 10 SigmaMCP tools with full signatures + 12 SigmaMemory tools
- `agent-roles-and-protocol.md` — 4 roles, 8 rules, file ownership, protocol fragments
- `skills-spec.md` — SigmaMind plugin marketplace + canonical Anthropic Skills cross-check
...
- SigmaSpace v3.0.x is the current line (initial v3.0.1 on 2026-04-20).
- SigmaVoice v2.2.22 current; v1.0.0 GA was 2026-02-23.
- Pricing: Basic $16/$20, Pro $40/$50, Ultra $80/$100; annual 20% off; 7-day refund.
- Credits per month: 5,000 / 12,500 / 25,000.
- SigmaMCP server: `https://mcp.sigmamind.ai/mcp`, API key `bm_live_*`.
