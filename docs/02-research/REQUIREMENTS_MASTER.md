...
- **`[CONFIRMED]`** — appears in SigmaMind official launch video transcript or video description (SigmaSpace official marketing).
- **`[INFERRED]`** — our own design call from the rebuild plan or research report (no direct SigmaMind source).
...
- `[CONFIRMED]` Operator picks a project directory before launching. `[video_transcript.txt §"SigmaSpace workspace demo"]`
- `[CONFIRMED]` Operator picks the count of agent terminal sessions to launch. `[video_transcript.txt §"SigmaSpace workspace demo"]`
- `[CONFIRMED]` Workspace can launch up to **16 simultaneous agent terminal sessions**, mixing Claude, Codex, and Gemini. `[video_transcript.txt §"SigmaSpace workspace demo"]`, `[docs/02-research/transcripts/launch-video-description.txt]`, `[research_extracted.txt §"SigmaSpace Feature Analysis / Core Architecture Overview"]`
- `[CONFIRMED]` Operator selects which agents (provider per pane) at launch time. Demo example mixed two Codex, one Gemini, one Cursor. `[video_transcript.txt §"SigmaSpace workspace demo"]`
- `[CONFIRMED]` "Workspace" is one of the two core products: SigmaSpace = isolated agents, SigmaSwarm = coordinating swarm. `[video_transcript.txt §"SigmaSwarm demo"]`
...
- `[INFERRED]` Three rooms: Command Room (terminals), Swarm Room (parallel agents), Review Room (oversight). `[research_extracted.txt §"SigmaSpace Feature Analysis / Core Architecture Overview"]`, `[REBUILD_PLAN.md §"Architecture (target)"]`
...
- `[CONFIRMED]` Roster preset selector ranges between 5 agents and 50 agents. `[video_transcript.txt §"SigmaSwarm demo"]`
- `[CONFIRMED]` Per-role provider mapping is operator-configurable. Speaker's defaults:
  - Coordinators → Codex.
  - Builders → Claude.
  - Scouts → Gemini.
  - Reviewers → Codex. `[video_transcript.txt §"SigmaSwarm demo"]`
- `[CONFIRMED]` Click "launch swarm" launches the configured swarm. `[video_transcript.txt §"SigmaSwarm demo"]`
- `[CONFIRMED]` SigmaSwarm differs from SigmaSpace: SigmaSpace = isolated agents; SigmaSwarm = coordinating swarm. `[video_transcript.txt §"SigmaSwarm demo"]`

### Swarm communication
- `[CONFIRMED]` Agent-to-agent messages visible in a side-chat panel. `[video_transcript.txt §"Live swarm demo"]`, `[video_transcript.txt §"SigmaSwarm demo"]`
...
- `[CONFIRMED]` Live coordination chat panel on the right side during a swarm run. `[video_transcript.txt §"SigmaSwarm demo"]`
- `[CONFIRMED]` Agents can ask the operator questions back (demo: "builder 2 asks the operator, 'How are you feeling?'"). `[video_transcript.txt §"SigmaSwarm demo"]`
- `[CONFIRMED]` SigmaSwarm is positioned for hand-off of long-running difficult tasks with minimal intervention. `[video_transcript.txt §"SigmaSwarm demo"]`, `[docs/02-research/transcripts/launch-video-description.txt]`

### Swarm prompt + knowledge upload
- `[CONFIRMED]` The swarm receives a free-text prompt at creation time. Demo: "identify any security vulnerabilities and fix them." `[video_transcript.txt §"SigmaSwarm demo"]`
- `[CONFIRMED]` Operator can upload PDFs, images, or arbitrary files into the "swarm's brain" as supporting knowledge. `[video_transcript.txt §"SigmaSwarm demo"]`
...
- `[INFERRED]` Orchestrator decomposes high-level tasks into sub-tasks for parallel execution. `[research_extracted.txt §"SigmaSpace Feature Analysis / Key Differentiators"]`
...
### Provider list
- `[CONFIRMED]` Claude Code, Codex, Gemini supported in SigmaSpace launch video. `[video_transcript.txt §"SigmaSpace workspace demo"]`
- `[CONFIRMED]` Cursor agent shown in demo agent mix. `[video_transcript.txt §"SigmaSpace workspace demo"]`
...
- `[OPEN]` Whether the browser pane has any presence in SigmaMind's SigmaSpace product (no mention in transcript / description) — design decision is wholly ours.
...
- `[OPEN]` SigmaSpace's official position on Anthropic Skills is not stated in any source. Skills support is our addition.

---

## Memory (SigmaMemory equivalent)

- `[CONFIRMED]` SigmaSpace has SigmaMemory: local-first knowledge graph with **12 MCP tools** for shared agent memory. `[research_extracted.txt §"SigmaSpace Feature Analysis / Core Architecture Overview"]`
- `[CONFIRMED]` Wikilink-based knowledge graph; supports orchestrator shared context. `[research_extracted.txt §"SigmaSpace Feature Analysis / Key Differentiators"]`
...
- `[CONFIRMED]` SigmaSpace has a Kanban Task Board with task-to-agent assignment. `[research_extracted.txt §"SigmaSpace Feature Analysis / Core Architecture Overview"]`
...
- `[CONFIRMED]` SigmaSpace requires the SigmaMind basic plan. `[video_transcript.txt §"Pricing"]`
...
- `[CONFIRMED]` Distribution: download from `sigmamind.ai`. `[video_transcript.txt §"Pricing"]`, `[docs/02-research/transcripts/launch-video-description.txt]`
...
- `[INFERRED]` SigmaMind-specific paid features. `[REBUILD_PLAN.md §"Out of scope"]`
...
- `[INFERRED]` No screenshot reproduction; no copy of proprietary SigmaSpace assets. `[REBUILD_PLAN.md §"Legal/IP guardrails"]`
...
- `[OPEN]` SigmaSpace's official Skills support and whether our Skills loader has any direct counterpart.
- `[OPEN]` Whether the in-app browser exists in SigmaSpace itself.
