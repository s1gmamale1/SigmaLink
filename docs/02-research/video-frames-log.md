# SigmaMind / SigmaSpace / SigmaSwarm — Video Frames Log

Chronological inventory of every visible UI state captured from public SigmaMind videos and thumbnails. Source-of-truth markers:
...
## Video 1 — "Introducing SigmaSpace | The Agentic Development Environment of the Future"

- **URL**: https://youtu.be/RG38jA-DFeM
- **Channel**: SigmaMind  (https://www.youtube.com/channel/UCwaTGE53GLGC3fDClVl_7TA)
- **Title (full)**: Introducing SigmaSpace | The Agentic Development Environment of the Future
...
- **YouTube chapters** (from JSON):
  - 00:00–00:20 Officially Launching SigmaSpace
  - 00:20–01:12 SigmaSwarm Live: Agents Coordinating in Real Time
  - 01:12–01:23 I Built SigmaSpace Using SigmaSpace
  - 01:23–02:03 SigmaSpace Workspaces: Mix Claude, Codex and Gemini
  - 02:03–02:28 SigmaSwarm Deep Dive: Coordinators, Builders, Scouts and Reviewers
...
- Right end of top bar: settings/gear icon, then a stylised lightning-bolt monogram (the SigmaMind brand mark, blue/cyan over dark).
...
- Each terminal pane shows a Claude Code splash:
  ...
  - Right of icon, text: `Claude Code v2.1.72` (bold), `Opus 4.6 with high effort`, `Claude Max`, `~/Desktop/sigmamind`.
  - Prompt line below: `matthewmiller@Mac-Studio sigmamind % cl` then wrapped `aude` (the user is typing the command `claude`).
- Middle of frame is occupied by a portrait of the founder ("Matthew Miller" per the path) and a glowing "4-pane app icon" (rounded-square dark icon with four white panels arranged 2×2, each panel showing 2-3 horizontal bars — this is the official SigmaSpace product icon). The glow behind the icon is amber/gold + cyan/blue.
- Burned-in title text "SIGMASPACE IS LIVE" in a bold sans-serif, with "SIGMASPACE" in a cyan→blue gradient and "IS LIVE" in white with black outline.

**00:00 – 00:20 — Officially Launching SigmaSpace [T]**
- VO: "Bridgepace is the agentic development environment of the future. After 145 days of vibe coding an app until I make a million dollars, Sigmamind is officially launching Bridgepace."
- Claim: SigmaSpace lets builders "ship code at the speed of thought".
- (No new UI described in this segment beyond the hero shot.)

**00:20 – 01:12 — SigmaSwarm Live demo [T+V partially via thumbnail glow]**
...
**01:12 – 01:23 — "I built SigmaSpace using SigmaSpace" interstitial [T]**
- Verbal claim only.

**01:23 – 02:03 — Creating a SigmaSpace workspace [T]**
- The presenter walks through the workspace-creation flow:
  1. "Navigate into whatever project directory I want" — the dialog has a directory picker. Demoed by `cd`-ing into `sigmamind`.
...
- SigmaSpace = workspace of *isolated* agents (each in its own terminal pane).

**02:03 – 02:28 — SigmaSwarm intro [T]**
...
- Basic plan = **$20 / month** to SigmaMind, includes SigmaSpace.
- Launch coupon: **`launch20`** for **20% off** ("for a limited time").
- CTA: `sigmamind.ai`.

---

## Video 2 — "Vibe Coding With SigmaSpace 3"

- **URL**: https://www.youtube.com/watch?v=xKf0B6AEo9I
- **Title**: Vibe Coding With SigmaSpace 3
- **Length**: 1085 s (≈ 18 min 5 s)
- **Upload date**: 2026-04-22
- **Transcript files**:
  - `transcripts/vibe-coding-sigmaspace-3-xKf0B6AEo9I.clean.txt`
  - `transcripts/vibe-coding-sigmaspace-3-xKf0B6AEo9I.en.vtt`
  - `transcripts/vibe-coding-sigmaspace-3-xKf0B6AEo9I.info.json`
- **Thumbnail files**: `thumbnails/vibe-coding-sigmaspace-3-xKf0B6AEo9I.webp` and `…-1080p.jpg`
- **YouTube chapters**:
  - 00:00–00:34 Intro: SigmaSpace 3 is Live on Product Hunt
  ...
  - 14:21–15:34 Live Test: The AI-Generated SigmaSpace Mobile App
...
**Thumbnail [V]**
- Two visual elements:
  1. **The SigmaSpace product icon (left)**: dark rounded-square icon, four white panels in a 2×2 grid, each panel showing 2–3 horizontal bars. Strong amber-orange glow on the left side of the icon, deep blue glow on the right side. Radial dark background.
  2. **Swarm grid panel (right)**: a wide dark "macOS-window-style" rectangle (rounded 8–12 px corners, thin amber/orange-to-blue glow border). Contents: a **4-column × 4-row grid of small agent cards**. Each card has a header line (~6-8 chars: "Agent…" + version number) and a small pixel-art crab icon (Claude Code mascot) plus 2-3 lines of dim grey text. Below each card is a 1-line status/log line. This is *the SigmaSwarm Operator Console / Command Room view* showing 12+ agents at once.
- Burned-in copy: "VIBE CODING WITH" (white) and "SIGMASPACE 3" with letter-by-letter gradient (B-R-I-D-G-E in amber/orange → S-P-A-C-E in steel-blue → "3" in vivid blue).
...
- Demoes mix: **1 Codex + 1 Claude** in the `sigmamind` directory.
...
- Voice prompt expanded to ~10 lines: build a React Native Expo app with NativeWind, follow SigmaMind UI theme/styling, integrate with SigmaMind API and dispatch to SigmaSpace Tauri desktop app, build welcome + login + dashboard.
...
- "Next" button → **Swarm Mission** input page.
  - Voice-prompt the mission via SigmaVoice (`@SigmaSpace Tauri` mention syntax for the target project).
...
- Any link in any agent terminal is **clickable** → opens inside a SigmaSpace-internal browser tab (multiple browser tabs supported in parallel, demoed `sigmamind.ai`, `openrouter.com`, X).
...
- The right-hand "IDE" tab is wired into terminal output: when an agent prints a file path, you can **click the path** and the file opens in the SigmaSpace editor.
...
- Sign-in screen → email continue → dashboard showing the user's SigmaMind plan ("pro plan rendered in") and *available actions* (`Swarm`, others). Some actions are non-functional "shipping in V4."
...
- Plan list shown:
  - **Basic plan** — 5,000 credits/month; SigmaMind account; access to SigmaSpace; access to multi-agent swarms; access to the **SigmaJarvis voice assistant**.
  - **Pro plan** — 12,500 credits/month; everything in Basic + **SigmaMind MCP** + **SigmaVoice** (voice-to-text).
- Coupon code **`V3`** = **50% off forever** at checkout (limited).
- Net price example: $10/mo for first 3 months on basic.

---

## Video 3 — "Testing GPT 5.4 With Agent Swarms In SigmaSpace"
...
- **YouTube chapters**:
  ...
  - 03:36–04:30 Launching Agent Swarms in SigmaSpace
...
- New-swarm flow:
  1. Click **`+`** or hit **`⌘T`** ("command T") to launch a new SigmaSpace OR a new SigmaSwarm.
  2. Pick SigmaSwarm.
  3. Type **swarm name** (e.g. *"next SAS dashboard"*).
  4. **`cd` into directory** (`cd desktop` → `cd sigmamind` → `cd GPT54 test`).
...
- Implication: there is also a **platoon** label (15 agents). "Legion" used colloquially for the very-large swarm. Cross-referencing the launch video's "5 or 50 agents" presets implies SigmaSpace ships at least these named tiers (verified visible: **Squad 5**, **Team ≈10**, **Platoon 15**, **Legion (≈30+ implied)**, **50-agent preset (mentioned only)**).

**09:50 – 12:35 — Multiple swarms in parallel [T]**
- Five swarms running concurrently in different tabs of SigmaSpace:
...
**14:15 – 16:50 — Classic SigmaSpace layout (terminals only) [T]**
- "Let's just launch four sessions and create a new bridge space" → SigmaSpace (non-Swarm) is described as "the classic layout" — 2×2 / 4-pane terminals.
...
- The user ships output by *clicking through* each generated app from inside SigmaSpace's built-in browser.
- Mentions a separate **bug bounty program** that pays in Bitcoin for reporting SigmaSpace bugs.
...
### Frames / UI states (SigmaSpace-relevant fragments only)

- 00:24 [T]: "Right here I'm using SigmaSpace and as you can see I have **six Claude Opus 4.6 terminals** opened up in SigmaSpace. This is actually an ADE product that we are working on right now …"
- 02:25 [T]: Demonstrates `Ctrl-T` (likely `⌘-T` on Mac) opens a new workspace dialog. Picks "single" → 1-pane SigmaSpace with 1 Claude Code instance. → confirms the single-pane workspace mode in V2.
- (Most of this video is *not* about SigmaSpace; it's about Anthropic's new "agent teams" feature inside Claude Code itself. Useful only to confirm that SigmaSpace was being used as a daily driver for 6 parallel Opus terminals before V3.)

---

## Video 5 — context-only references

The clean transcript for `openclaw-strategy-141n8k-5K14.clean.txt` ("My OpenClaw Strategy Starts Today") mentions SigmaMind / SigmaMind hoodies / Mac mention but contains no new SigmaSpace UI surface; not material for this log.

Other channel videos that *probably* contain more SigmaSpace footage but were not pulled this pass (saving on rate-limit budget): "Vibe Coding With Cursor 3", "Vibe Coding With Claude Sonnet 4.6", "Vibe Coding With Opencode", "Vibe Coding With Composer 2", "Vibe Coding With Cursor Cloud Agents", "Vibe Coding With Grok 4.3 in a Full Self Driving Tesla", "5 Things I've Learned After 154 Days …", "Vibe Coding With Claude Code Desktop App", "How Claude Code Stopped A DDoS Attack", "Testing The New Claude Code Rate Limits", "Vibe Coding With Obsidian", "Vibe Coding With SigmaSpace 3" (already done), "Officially Launching SigmaVoice". A complete pull is queued in `open-video-questions.md`.
