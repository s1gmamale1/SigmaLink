; installer.nsh — NSIS custom welcome page for BridgeVoice
; Mirrors app/build/installer.nsh with BridgeVoice-specific text.
; Shows the SmartScreen / Mark-of-the-Web workaround during installation.

!define MUI_WELCOMEPAGE_TITLE "Installing BridgeVoice — please read"
!define MUI_WELCOMEPAGE_TEXT \
  "BridgeVoice ships without an Authenticode code-signing certificate, so the$\r$\n\
   ...
   $\r$\n\
     ...
     2. The dialog expands and shows a $\"Run anyway$\" button at the$\r$\n\
        bottom. Click it.$\r$\n\
     Done. BridgeVoice installs / launches. No more prompts.$\r$\n$\r$\n\
   OPTION B — Unblock the file before running (no SmartScreen prompt)$\r$\n\
   $\r$\n\
     1. Locate the downloaded `BridgeVoice-Setup-*.exe` in File Explorer.$\r$\n\
   ...
   $\r$\n\
     ...
     organisational identity verification. BridgeVoice is currently internal-use$\r$\n\
