...
// BUG-V1.1.2-01: Jorvis Assistant MCP host stdio server entry. Spawned by the
// Claude CLI via `--mcp-config <tmp>.mcp.json` so the CLI sees the 13 Jorvis
// tools natively (and emits real `tool_use` envelopes). The banner sets
// JORVIS_HOST_AUTOBOOT=1 so the CJS file auto-boots on direct invocation,
...
esbuild.buildSync({
  ...
  format: 'cjs',
  outfile: path.join(outDir, 'mcp-jorvis-host-server.cjs'),
