// P3-S7 — Jorvis → swarm cross-link banner.
//
// When a swarm was created via the Jorvis Assistant `create_swarm` tool, the
...
// the active room back to `jorvis`, asks JorvisRoom to load the originating
...
export function OriginLink({ swarmId }: Props) {
  ...
  const onJump = () => {
    // 1) Switch the room. 2) Notify JorvisRoom via a window event so it
  ...
  return (
    ...
    >
      ...
      <span>
        Started from{' '}
        <span className="font-medium text-foreground">Jorvis chat</span>
        {' · '}
