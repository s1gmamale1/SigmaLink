...
const TITLE_LEAD = /^(can|could|would|will|please|pls|kindly|hey|yo|ok|okay|so|now|first|just|i|i'd|i'll|we|let|lets|let's|help|wanna|want|need|give|gimme|show|tell|do)\b[\s,'-]*/i;
// Low-value connectors dropped from the middle of the title (kept in slots 0-1).
const TITLE_STOP = new Set(['the', 'a', 'an', 'to', 'for', 'of', 'with', 'and', 'or', 'me', 'my', 'your', 'please', 'about', 'that', 'this', 'it', 'on', 'in', 'at', 'is', 'are', 'be', 'as', 'by', 'from', 'into', 'up']);

/** Instant, free, model-less title from a prompt — peel filler, take the first
 *  clause's significant words, Title-Case, cap. The renderer floor that shows the
 *  moment the operator hits Enter (Cursor's strategy); an LLM/SIGMA::LABEL title
 *  upgrades it later. Always returns a non-empty string. */
export function heuristicTitle(prompt: string | null | undefined): string {
  ...
  for (let i = 0; i < 6; i++) { const n = s.replace(TITLE_LEAD, ''); if (n === s) break; s = n; }
  const clause = s.split(/[,.;:\n!?]/)[0].trim();
  const base = clause.length >= 12 ? clause : s;
  const words = base
    .split(' ')
    .filter(Boolean)
    .filter((w, i) => i < 2 || !TITLE_STOP.has(w.toLowerCase()));
  let t = words.slice(0, 5).join(' ').replace(/[^\p{L}\p{N}\s'-]/gu, '').replace(/\s+/g, ' ').trim();
  t = t.replace(/\b\p{L}/gu, (c) => c.toUpperCase());
  t = t.length > PROMPT_CAP ? t.slice(0, PROMPT_CAP).trim() : t;
  return t || summarizePrompt(prompt) || 'New task';
}
