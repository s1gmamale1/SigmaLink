...
| Linux x64 AppImage + deb | Built by [`../.github/workflows/release-linux.yml`](../.github/workflows/release-linux.yml) on every `v*` tag push (also `workflow_dispatch`). Ubuntu 22.04/24.04 x64 is smoke-tested under Xvfb in `e2e-matrix.yml`. Auto-update is a manual download + reveal-in-folder (no in-place apply). |
| Linux one-line installer | [`scripts/install-linux.sh`](scripts/install-linux.sh). POSIX Bash, guards Ubuntu 22.04/24.04 x64, downloads the `.deb` from GitHub Releases, and installs via `apt-get`. |
