...
3. **electron-builder** — consumes both outputs and produces installers. Configuration lives in [`electron-builder.yml`](electron-builder.yml). Targets: macOS arm64 (DMG + zip) and Windows x64 (NSIS + portable). Intel macOS and Linux artefacts were retired in v3.1.0.
...
| ~~Linux AppImage + deb~~ | **Retired in v3.1.0** (last shipped `v3.0.0`). `release-linux.yml` and `scripts/install-linux.sh` are deleted. Artefacts only — the ubuntu jobs in `e2e-matrix.yml` and `lint-and-build.yml` still run on every PR, and `pickLinuxAppImage()` + the `linux` updater branch remain for self-built trees. |
| ~~macOS Intel (x64) DMG~~ | **Retired in v3.1.0** (last shipped `v3.0.0`). `mac.target` is `arm64` only. A client on `≥ v3.1.0` running Intel gets an explicit unsupported-platform message; one on `≤ v3.0.0` predates `pickMacDmg()` and would grab the arm64 DMG it cannot execute. |
...
The historic Windows `.cmd` shim spawn bug ("Cannot create process, error code: 2") was resolved in v1.1.x — see [`../docs/01-investigation/01-known-bug-windows-pty.md`](../docs/01-investigation/01-known-bug-windows-pty.md) for the full investigation and the patch landing record. Voice capture on Windows runs on a native SAPI5 (`ISpRecognizer`) binding — `@sigmalink/voice-win`, shipped in v1.5.0 and hardened through v1.5.3 — and falls back to the Chromium Web Speech API (requires internet) only when the native module cannot load. Linux has no native recogniser and always uses the Web Speech fallback. The full Windows-port design including all platform trade-offs lives at [`../docs/04-design/windows-port.md`](../docs/04-design/windows-port.md).
